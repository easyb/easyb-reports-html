...
scenario "a passing, failing and pending specification", {
    ...
    when "the reports are written", {
        htmlReportLocation = "${reportsDir}/PassingPendingFailing-specification-report.html"
        reportWriter = new HtmlReportWriter()
        reportWriter.location = htmlReportLocation
        reportWriter.writeReport(new ResultsAmalgamator(storyBehavior))
    }
...
scenario "a passing, failing and pending scenario", {
    given "a story file with a passing, failing and pending scenario is loaded", {
        storyBehavior = BehaviorFactory.createBehavior(new File("${testSourceDir}/html/PassingPendingFailingStory.story"))
    }
    ...
    when "the html reports are written", {
        htmlReportLocation = "${reportsDir}/PassingPendingFailingStory-story-report.html"
        htmlReportFile = new File(htmlReportLocation)
        reportWriter = new HtmlReportWriter()
        reportWriter.location = htmlReportLocation
        reportWriter.writeReport(new ResultsAmalgamator(storyBehavior))
    }
    ...
    then "the resulting html should have 3 total behaviors in the behavior summary", {
        xmlReport = new XmlSlurper(new org.ccil.cowan.tagsoup.Parser()).parse(htmlReportFile)
        contentDiv = findContentDiv()
        summariesDiv = findSummariesDiv()
    ...
    then "should have a stories list with a story name passing pending failing", {
        ...
        storyRow.td[0].a.text().shouldBe 'passing pending failing story'
    }
    ...
    then "should have a scenario named failing scenario", {
        ...
        failingScenarioComponentDetailsRow.td.strong.text().shouldBe "org.easyb.exception.VerificationException: expected 0 but was 1"
